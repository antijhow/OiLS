...
plot(x, y, lty=1, col = "grey",type="l",main="N=200",xlim=c(0,1),ylim=c(-10,15),xlab="",ylab="",cex.main = 3,cex.axis=1.5)              # Create first plot

par(new = TRUE)                             # Add new plot
plot(density(var.break),xlim=c(0,1), lty=2,lwd=4,axes=FALSE,xlab="",ylab="",col="red",main="",ylim=c(0,6),cex.main =3,cex.axis=1.5)
lines(density(mean.break2),lty=5,col="blue",lwd=4)
abline(v=a,lty=4,col="orange",lwd=4)
abline(v=b,lty=3,col="green",lwd=4)
axis(side = 4, at = c(0,2,4,6),cex.axis=1.5)      # Add second axis
