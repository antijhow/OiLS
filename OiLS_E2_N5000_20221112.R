...
for (i in 1:loop)
{
	...
	#M2.sq.res = (y - M2.y)^2
	#plot(x,M2.sq.res,main="Experiment I",type="p",xlab="",ylab="",cex.axis=1.5,cex.main=1.5)
...
plot(x, y, lty=1, col = "grey",type="l",main="N=5000",xlim=c(-4,4),ylim=c(-20,30),xlab="",ylab="",cex.main=3,cex.axis=1.5)              # Create first plot

par(new = TRUE)                             # Add new plot
plot(density(mean.break),xlim=c(-4,4), lty=2,lwd=4,axes=FALSE,xlab="",ylab="",col="red",main="")
lines(density(var.break),lty=5,col="blue",lwd=4)
abline(v=a,lty=4,col="orange",lwd=4)
axis(side = 4, at = c(0,6,12,18),cex.axis=1.5)      # Add second axis
