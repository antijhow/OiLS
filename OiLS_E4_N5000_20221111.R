...
cut.set = c(0.2,0.35,0.5,0.65,0.8)
M = length(cut.set)
coef.matrix = matrix(runif((p+1)*(M+1),-3,3),p+1,M+1)
...
plot(x, y, lty="solid", col = "grey",type="l",main="N=5000",ylim=c(-3,5),cex.main=3,cex.axis=1.5,ylab="",xlab="")              # Create first plot
...
length(which(model.cut.5[,2]==2))/loop
length(which(model.cut.5[,3]==2))/loop
length(which(model.cut.5[,4]==2))/loop
length(which(model.cut.5[,5]==1))/loop
